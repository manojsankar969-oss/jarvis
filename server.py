...
if IS_MACOS:
    ...
    # Windows/Linux stubs
    async def execute_action(*args, **kwargs): return {"success": False, "confirmation": "Action not available on this platform"}
    async def monitor_build(*args, **kwargs): return
    def open_terminal(*args, **kwargs): return {"success": False, "confirmation": "Terminal control not available on Windows"}
    def open_browser(*args, **kwargs): return {"success": True, "confirmation": "Browser opened"}
    def open_claude_in_project(*args, **kwargs): return {"success": False, "confirmation": "Claude terminal not available on Windows"}
    def _generate_project_name(*args, **kwargs): return "Unknown"
    def prompt_existing_terminal(*args, **kwargs): return None

    async def get_active_windows(): return []
    async def take_screenshot(): return None
    def describe_screen(*args, **kwargs): return "Screen description not available on Windows"
    def format_windows_for_context(*args, **kwargs): return "No window information available"

    def get_todays_events(*args, **kwargs): return []
    def get_upcoming_events(*args, **kwargs): return []
    def get_next_event(*args, **kwargs): return None
    def format_events_for_context(*args, **kwargs): return "No calendar data available"
    def format_schedule_summary(*args, **kwargs): return "No calendar data available"
    def refresh_calendar_cache(): pass

    def get_unread_count(*args, **kwargs): return 0
    def get_unread_messages(*args, **kwargs): return []
    def get_recent_messages(*args, **kwargs): return []
    def search_mail(*args, **kwargs): return []
    def read_message(*args, **kwargs): return None
    def format_unread_summary(*args, **kwargs): return "No mail data available"
    def format_messages_for_context(*args, **kwargs): return "No mail data available"
    def format_messages_for_voice(*args, **kwargs): return "No mail data available"

    def get_recent_notes(*args, **kwargs): return []
    def read_note(*args, **kwargs): return None
    def search_notes_apple(*args, **kwargs): return []
    def create_apple_note(*args, **kwargs): return {"success": False, "confirmation": "Notes not available on Windows"}
