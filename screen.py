...
Two capabilities:
1. Window/app list via AppleScript (macOS) or PowerShell (Windows)
2. Screenshot via screencapture (macOS) or PIL/screenshot tools (Windows)
"""
...
async def get_active_windows() -> list[dict]:
    ...
    Uses AppleScript + System Events (macOS) or PowerShell (Windows).
    Returns list of {"app": str, "title": str, "frontmost": bool}.
    """
    if IS_WINDOWS:
        # Windows: use PowerShell to get window info
        try:
            ps_script = """
...
'@
$windows = @()
$proc = [Win32+EnumWindowsProc]{ param($hWnd, $lParam)
    if ([Win32]::IsWindowVisible($hWnd)) {
        $sb = New-Object System.Text.StringBuilder 256
        [Win32]::GetWindowText($hWnd, $sb, 256) | Out-Null
        $title = $sb.ToString()
        if ($title -ne "") {
            $windows += "cmd|||" + $title + "|||false"
        }
    }
    return $true
}
                ...
                if "|||" in line:
                    ...
                    if len(parts) == 3:
                        windows.append({
                            "app": parts[0].strip(),
                            "title": parts[1].strip(),
                            "frontmost": parts[2].strip() == "true",
                        })
            return windows
        except Exception as e:
            log.warning(f"get_active_windows failed on Windows: {e}")
            return []
...
"""
    try:
        ...
        log.warning(f"get_active_windows failed: {e}")
        return []
