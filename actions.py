"""
JARVIS Action Executor — System actions for macOS and Windows.
...
async def open_browser(url: str, browser: str = "chrome") -> dict:
    ...
    if IS_WINDOWS:
        ...
        try:
            ...
            return {"success": True, "confirmation": f"Pulled that up in your browser, sir."}
        except Exception as e:
            log.error(f"open_browser failed on Windows: {e}")
...
async def open_claude_in_project(project_dir: str, prompt: str) -> dict:
    ...
    if IS_WINDOWS:
        # Windows: use cmd to launch claude
        try:
            proc = await asyncio.create_subprocess_exec(
                "cmd", "/c", "start", "cmd", "/k",
            ...
            log.error(f"open_claude_in_project failed on Windows: {e}")
...
async def prompt_existing_terminal(project_name: str, prompt: str) -> dict:
    ...
    if IS_WINDOWS:
        # Windows: stub implementation (would need more complex logic with win32API)
        return {
            "success": False,
            "confirmation": f"Terminal interaction not yet supported on Windows, sir.",
        }

    # macOS: use Terminal.app and System Events
...
async def get_chrome_tab_info() -> dict:
    ...
    if IS_WINDOWS:
        # Windows: stub implementation (would need COM automation or similar)
        return {}
